    ...
    <div className="space-y-6">
            ...
            <div key={a.id} className={`card border-l-4 ${priorityBorder(a.priority)} hover:shadow-md transition-all duration-300 animate-slide-up`} style={{ animationDelay: `${i * 80}ms` }}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <h3 className="text-lg font-semibold text-slate-800 dark:text-white mb-1">{a.title}</h3>
                  <p className="text-sm text-slate-600 dark:text-slate-400 whitespace-pre-line">{a.content}</p>
                </div>
                {isAdmin && (
                  <div className="flex gap-1 shrink-0">
                    <button onClick={() => openEdit(a)} className="p-1.5 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded transition">
                      <Edit2 size={15} />
                    </button>
                    <button onClick={() => handleDelete(a.id)} className="p-1.5 text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded transition">
                      <Trash2 size={15} />
                    </button>
                  </div>
                )}
      ...
      {showModal && (
        <>
          ...
          <div className="slide-panel slide-panel-active bg-white dark:bg-slate-800 shadow-2xl z-50 max-w-md">
